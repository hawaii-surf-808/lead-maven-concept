...
import { 
  ...
  Search, Plus, Bell, TrendingUp, BarChart, X, Menu,
  Download, ArrowUpRight, ArrowDownRight, Sparkles, MoveRight, Star
} from 'lucide-react';
...
const SidebarItem = ({ icon: Icon, label, active = false }: { icon: any, label: string, active?: boolean }) => (
  <button className={`flex w-full items-center gap-3 px-4 py-2.5 rounded-lg mb-1 transition-colors ${
    active 
        ...
        <div className="flex-1 overflow-y-auto p-4 py-6">
          <div className="space-y-6">
            <div>
              ...
              <SidebarItem icon={GitMerge} label="Pipeline" />
              <SidebarItem icon={Users} label="Customers" />
            </div>
            <div>
              <SidebarItem icon={MessageSquare} label="Conversations" />
              <SidebarItem icon={Contact} label="Contacts" />
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden relative">
        ...
        <header className="h-20 flex items-center justify-between px-6 lg:px-10 border-b border-border-subtle bg-surface-base/80 backdrop-blur-md sticky top-0 z-30">
          ...
          <div className="flex items-center gap-6">
            {/* Theme Toggle (Hidden functionally in mockup, but added for the user's request) */}
            <button 
              onClick={() => setIsDark(!isDark)}
              className="text-text-tertiary hover:text-text-primary transition-colors text-sm font-medium border border-border-subtle px-3 py-1.5 rounded-full"
            >
